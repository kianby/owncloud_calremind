...
if [ "$KIND" = "weekly" ]; then
    query="SELECT summary,startdate FROM oc_clndr_objects WHERE calendarid = $CALID AND startdate BETWEEN DATE(CURDATE()) AND DATE_SUB( CURDATE(), INTERVAL -7 DAY ) ORDER by startdate \G"
